...
export const Thumbnail = ({src,fallback,isLive,username} : ThumbnailProps) => {
    ...
    if(!src){
        content = (
            <div className="bg-[#2a2c37] flex flex-col items-center justify-center gap-y-4 h-full w-full transition-transform group-hover:translate-x-2 group-hover:translate-y-2 rounded-md border">
                <UserAvatar size={'lg'} 
                showBadge
                username={username}
                imageUrl={fallback}
